...
std::tuple<std::vector<int>, double> mid_to_vector1(const std::string& filename, double min_duration = INTMAX_MAX)
{
    using namespace smf;
...
template <class It>
int lcs_mod(It x_begin, It x_end, It y_begin, It y_end)
{
    auto m = std::distance(x_begin, x_end);
    auto n = std::distance(y_begin, y_end);
